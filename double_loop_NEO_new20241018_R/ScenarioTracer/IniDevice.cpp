...
/* 静的インスタンス */
GyroSensor  mResetRollAngle(PORT_4);                // リセットロール角
Motor       mStopForLeftMotor(PORT_C);              // 停止用左モータ
Motor       mStopForRightMotor(PORT_B);             // 停止用右モータ
Motor       mResetForLeftMotorEncoder(PORT_C);      // リセット用左モータエンコーダ
Motor       mResetForRightMotorEncoder(PORT_B);     // リセット用右モータエンコーダ
...
void IniDevice::resetMotorEncoder()
{
    ...
    {
        mResetForLeftMotorEncoder.setCount(0);
        mResetForRightMotorEncoder.setCount(0);
    }
}
